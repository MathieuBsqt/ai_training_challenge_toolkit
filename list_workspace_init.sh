...
jq -n --argfile jobs <( ovhai job list  -o json )  --argfile tokens <( ovhai token list  -o json ) -r 'include "lib"; list_running_ids($jobs;$tokens)' | xargs -n 1 -L 1 -I {} ovhai job exec -- {} bash -c 'echo {},$(ps -fu ovh | grep jupyter | grep -v grep | wc -l)' |
